...
// Problem 1
    // Yodo is a sponser of the fun run and the runners who work there get a fancy shirt we need a list of those runners to give them the special shirt
const yodoRunners = runners.filter(runner => runner.company_name === "Yodo")

// Problem 2
    // EMS is on scene for the fun run but we need to be sure we have enough people
    // big people are more likely to have heart attacks so lets se how many big people we got to make sure we got enough emergency personel
const bigShirts = runners.filter(runner => runner.shirt_size.includes("X")).map(runner => `${runner.first_name} ${runner.last_name}`);
console.log(bigShirts);

// Problem 3
    // there was a competition to see what company brought the most employees so lets see which employer brought the most people
const countEmployer = {};

runners.forEach(runner => {

    let emps = Object.keys(countEmployer);

    if (!emps.includes(runner.company_name)) { 
        countEmployer[runner.company_name] = 0;
    }

    countEmployer[runner.company_name] += 1;
});

let empCountArray = Object.entries(countEmployer);

empCountArray.sort((item1, item2) => {
    return item1[1] > item2[1] ? -1 : 1;
});

console.log(empCountArray[0]);
